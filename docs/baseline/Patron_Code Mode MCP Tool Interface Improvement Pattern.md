...
source_file: docs/baseline/Patron_Code Mode MCP Tool Interface Improvement Pattern.md
...
Fuente histórica sobre orquestación en modo código para MCP. La superficie operativa curada está en la [Fuente consolidada de `docs/baseline`](../01-CAPTURES/patterns/20260513-172100-baseline-source-consolidated.md).

## Síntesis

El patrón plantea que, para flujos multi-paso, Claude rinde mejor escribiendo código que orquesta herramientas MCP que invocándolas una por una desde el contexto.

LLMs generate TypeScript code to orchestrate MCP tools in ephemeral V8 isolates, eliminating token-heavy round-trips and enabling efficient multi-step workflows with 10x+ token savings.
